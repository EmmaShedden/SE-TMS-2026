...
if (TRUE) {
  ...
  logtime_outputdir = 
    sprintf("%s/threeback_v9_accur1_model_summaries", outputdir)
...
fin = sprintf("%s/threeback_v9_accur1_model_summaries/metrics.csv", outputdir)
...
BIC and AIC favor different models so we check

The variance point estimates shown here feed directly into the PVE below.

```{r}
m0 = glmer(fB, family=binomial, data=data)
summary(m0)
```

```{r}
m1 =  glmer(fA, family=binomial, data=data)
summary(m1)
```
...
m1: (correct_answer) \~ session + (1 \| participant_id) + (1 \| participant_id:shown_letter:TMS_Type) is prefered due to p\<0.05
...
The person-conditon-letter effect does excludes zero so it is significant.
...
get_pve(m1, "participant_id:shown_letter:TMS_Type")
```

The percent impact is 33% for non-systematic TMS effect that is person-dependednt and letter-dependent
